...
export default async function handler(req, res) {
  try {
    ...
    if (!row.device_token) {
      const newToken = deviceToken || randomBytes(24).toString('hex')
    ...
    // Jaring pengaman terakhir: apapun yang gagal tak terduga, tetap balas JSON yang jelas
    // (bukan halaman error mentah) supaya AccessGate tidak menampilkan pesan "gagal konek" yang kosong.
    console.error('verify-access error:', err)
    return res.status(200).json({ ok: false, message: `Terjadi kesalahan server: ${err?.message || 'tidak diketahui'}. Hubungi penjual.` })
  }
}
